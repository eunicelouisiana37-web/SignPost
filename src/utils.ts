...
export function exportToPDF(
  ...
  doc.setFontSize(22);
  doc.text(`${orgName.toUpperCase()} — STAFF REGISTRY`, 14, 20);
  
  doc.setFont('Helvetica', 'normal');
  doc.setFontSize(10);
  doc.setTextColor(240, 240, 240);
  doc.text('Staff Attendance & Payroll Timesheet Export', 14, 30);
  ...
  doc.text(`Staff Group: ${staffFilter || 'All Staff'}`, 14, 68);
  doc.text(`Exported On: ${new Date().toLocaleDateString('en-GB')}`, 14, 74);
  
  // Core Statistics calculator
  const totalWages = records.reduce((sum, r) => {
    const member = staffList?.find(s => s.id === r.staffId);
    const hourly = member?.hourlyRate ?? 3500;
    const ot = member?.overtimeRate ?? 5250;
    return sum + (r.regularHours * hourly + r.overtimeHours * ot);
  }, 0);
  ...
  doc.text('Official payroll timesheet generated by SignPost Pro. Verify records with your HR administrator.', 14, 286);
  const docId = `DOC-${Date.now().toString(36).toUpperCase()}-${Math.random().toString(36).slice(2,6).toUpperCase()}`;
  doc.text(`Document ID: ${docId} | Generated: ${new Date().toISOString()}`, 14, 291);
